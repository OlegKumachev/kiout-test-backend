...
from workers.serializers import WorkerSerializers, WorkerDetailSerializer, ImportWorkersSerializer
...
class WorkersViewSet(ModelViewSet):
    ...
    @extend_schema(
        summary="Импорт работников из Excel",
        description="Загрузите Excel файл для импорта сотрудников",
        request={
            'multipart/form-data': {
                'type': 'object',
                'properties': {
    ...
    def import_workers(self, request):
        ...
        try:
            ...
            result = resource.import_data(
                dataset,
                dry_run=False,
                raise_errors=False,  # не прерывать при ошибках
